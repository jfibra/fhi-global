...
Google Sign-In (`components/auth/GoogleAuthFlow.tsx` on `/login` + `/register`) uses Supabase's `signInWithOAuth` redirect flow (not a popup — reliable regardless of third-party-cookie settings): click → Google → `app/auth/callback/route.ts` (`exchangeCodeForSession`) → `app/auth/google/continue` immediately calls `app/api/auth/google/finalize`, which provisions the FHI profile via the service-role client (name/avatar from Google, referral attribution) — every new account is `member` + `pending`, exactly like self-registration; there is no external role lookup. The write is guarded by `metadata.google_provisioned` so returning users and admin role changes are never overwritten. (The former Leuterio Realty lookup/import — `lib/lr/*`, `app/api/lr/lookup`, `LrAccountModal` — was removed; roles are granted only by admins or the Invite ladder.) `lib/auth.ts` least-privilege defaults (migration 007: role `member`, status `pending`) mean a skipped/failed provisioning step never yields elevated access.
