...
export function EventCertificateModal({
  ...
  }, [selfService, selfServiceUrl, event.title, event.brand])
      ...
      >
        ...
        <div className="mt-6 border-t border-[#f0f0f0] pt-5">
          <div className="grid gap-5 lg:grid-cols-[1fr_260px]">
            ...
            <div className="flex flex-col gap-2 border border-[#e5e5e5] p-4 bg-[#fafafa]">
              <p className={labelCls}>Poster for the venue screen</p>
              <div className="flex gap-3">
                <div className="w-[118px] shrink-0 aspect-[9/16] bg-[#001f3f] border border-[#e5e5e5] overflow-hidden flex items-center justify-center">
                  {posterThumb ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={posterThumb} alt="Certificate QR poster preview" className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="flex-1 flex flex-col gap-1.5 min-w-0">
                  <p className="text-[11px] text-[#6b7280] leading-snug">
                    Branded “Scan to get your certificate” poster with the QR, the seal and the three steps. Show it on the screen or print it.
                  </p>
                  <button type="button" onClick={() => void downloadPoster("story")} disabled={posterBusy !== null}
                    className="inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-[#001f3f] text-white text-xs font-bold hover:bg-[#00305f] disabled:opacity-50">
                    {posterBusy === "story" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />} Screen · 1080×1920
                  </button>
                  <button type="button" onClick={() => void downloadPoster("a4")} disabled={posterBusy !== null}
                    className="inline-flex items-center justify-center gap-1.5 px-3 py-2 border border-[#e5e5e5] bg-white text-xs font-bold text-[#374151] hover:border-[#001f3f] disabled:opacity-50">
                    {posterBusy === "a4" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />} Print · A4
                  </button>
                  <button type="button" onClick={() => { void navigator.clipboard.writeText(selfServiceUrl); setCopied(true); setTimeout(() => setCopied(false), 1500) }}
                    className="inline-flex items-center justify-center gap-1.5 px-3 py-2 border border-[#e5e5e5] bg-white text-xs font-bold text-[#6b7280] hover:border-[#001f3f]">
                    <Copy className="w-3.5 h-3.5" /> {copied ? "Copied" : "Copy link"}
                  </button>
                  <div ref={qrRef} className="hidden"><QRCodeCanvas value={selfServiceUrl} size={64} /></div>
                </div>
              </div>
              <p className="text-[10px] text-[#9ca3af] break-all inline-flex items-center gap-1"><QrCode className="w-3 h-3 shrink-0" /> {selfServiceUrl}</p>
            </div>
          </div>
        </div>
