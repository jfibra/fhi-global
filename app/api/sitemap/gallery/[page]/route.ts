...
/** /sitemap-gallery-N.xml — published gallery albums (detail route resolves by slug only). */
...
export async function GET(_req: Request, ctx: { params: Promise<{ page: string }> }) {
  ...
  const rows = await fetchSectionPage("gallery", pageNum)
